...
def bar_plot_cs(df, value, location,tab_string):
    ...
    return fig_cs
####################################################
def pie_func(df,value_pie,cs_name,tab_string):
    dff5 = df.copy()
    dff5 = dff5[dff5['Year'] == value_pie]

    if tab_string == 'Share of renewables in electricity production (%)':
        dff5['Share of non renewables in electricity production (%)'] = 100 - dff5[tab_string]

        labels_dict ={tab_string : 'Renewable Energy',
            'Share of non renewables in electricity production (%)': 'Non Renewable Energy'}

        if cs_name == 1:
            dff6 = dff5.copy()
            dff6 = dff6[[tab_string,'Share of non renewables in electricity production (%)' ]]
            dff6 = dff6.sum()

            piechart=px.pie(
                    data_frame=dff6,
                    values = dff6.values.tolist(),
            ...
            dff5 = dff5[dff5['Country'] == cs_name]
            dff5[[tab_string,'Share of non renewables in electricity production (%)']]

            piechart=px.pie(
                        data_frame=dff5,
                        values = dff5.values.tolist()[0],
                        names= list(labels_dict.values()),
                        hole=.8,
                    color_discrete_sequence = px.colors.qualitative.Antique)
            piechart.update_layout({"plot_bgcolor": "rgba(0, 0, 0, 0)",
        ...
        dff5['Share of others in total final energy consumption (%)'] = 100 - dff5[tab_string]
        labels_dict ={tab_string : 'Eletricity',
            'Share of others in total final energy consumption (%)': 'Other'}

        if cs_name == 1:
            dff6 = dff5.copy()
            dff6 = dff6[[tab_string,'Share of others in total final energy consumption (%)' ]]
            dff6 = dff6.sum()
            ...
            dff5 = dff5[dff5['Country'] == cs_name]
            dff5[[tab_string,'Share of others in total final energy consumption (%)%)']]
...
layout = html.Div([
    ...
    html.Div(children=[
                    ...
                    dcc.Slider(id='EL_simple_slider',
                            min = 1990, 
                            max = 2020, 
                            step = 1, 
                            value=1990,  
    ...
    html.Div(children=[
                        dcc.Graph(id="EL_bar_plot_2_2", style={'display': 'inline-block','width': '64%'}),
                        dcc.Graph(id="EL_circle_plot", style={'display': 'inline-block','width': '34%'}),
                        ])
...
@callback(
    ...
    Output(component_id ='EL_circle_plot',component_property = 'figure'),
    ...
    Input(component_id = 'EL_simple_slider', component_property = 'value'),
    Input(component_id ='EL_world_plot',component_property = 'clickData'),

    ])
def update_graph(selected_year,Share_of_Renewables,Share_of_Electricity, sort_button_value,sort_button2_value,value_bar,value_pie,c_selection):
    dff = df.copy()
...
########################################## Barchart ##############################################################################################
    ...
    if c_selection is not None:
        ...
        piechart = pie_func(df,value_pie,cs_name,tab_string)
        
        return header, button_text , button2_text, fig, bar_hor, figure1, piechart
    else:
        #fig2 =  Continent_comp(df, tab_string,value_bar)
        piechart = pie_func(df,value_pie,1,tab_string)

        return header, button_text , button2_text, fig, bar_hor, fig2, piechart